...
where
    ...
    FN: FnMut(&N) -> IN,
    IN: IntoIterator<Item = (N, C, usize)>,
    FH: FnMut(&N) -> C,
    FS: FnMut(&N) -> bool,
{
    ...
    while let Some(SmallestCostHolder { cost, index, .. }) = to_see.pop() {
        ...
        for (successor, mut move_cost,priority) in successors {
...
impl<K: PartialEq> PartialEq for SmallestCostHolder<K> {
    fn eq(&self, other: &Self) -> bool {
        self.estimated_cost.eq(&other.estimated_cost) && self.cost.eq(&other.cost) && self.priority.eq(&other.priority)
    }
}
...
impl<K: Ord> Ord for SmallestCostHolder<K> {
    fn cmp(&self, other: &Self) -> Ordering {
        match other.estimated_cost.cmp(&self.estimated_cost) {
            Ordering::Equal => self.cost.cmp(&other.cost).then_with(|| self.priority.cmp(&other.priority)),
            s => s,
        }
    }
}