use crate::{
    ...
    ebi_objects::{labelled_petri_net::LabelledPetriNet, process_tree::ProcessTree, stochastic_transition_system::StochasticTransitionSystem},
    ebi_traits::{ebi_trait_event_log::EbiTraitEventLog, ebi_trait_finite_stochastic_language::EbiTraitFiniteStochasticLanguage},
    techniques::{
...
pub const EBI_DISCOVER_SDFA: EbiCommand = EbiCommand::Command { 
    ...
    input_names: &["EVENT_LOG_FILE", "EVENT_LOG_FILE", "TARGET_SIZE"], 
    input_helps: &[ "An event log.",  "An event log.", "The target size of the sdfa."], 
    execute: |mut inputs, _| {
        ...
        let mut stochastic_language: Box<dyn EbiTraitFiniteStochasticLanguage> = inputs
        .remove(0)
        .to_type::<dyn EbiTraitFiniteStochasticLanguage>()?;
        // stochastic_language.translate_using_activity_key(log.);
        ...
        let sdfa = convert_log_to_stochastic_transition_system(stochastic_language, sts, *target_size, size_before_filtering)?;
